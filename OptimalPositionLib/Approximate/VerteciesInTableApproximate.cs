...
namespace OptimalPositionLib
{
    ...
    public class VerticesInTableApproximate : VerteciesInTableDefine
    {
        ...
        //}
        //--------------------------------------------------------------------------------------
        public VerticesInTableApproximate(IGraph<IVertex> pGraph, IMatrixOfDistance pMatrixofDistance, string pAlgorithmName, int pMaxDurability)
        ...
        public override void ExecuteBody()
        {
            ...
            for (int i = 0; i < fOptimalRoute.Length; i++)
                fOptimalRoute[i] += 1;
            fOptimalWeight = ComputeWeightByPlace(OptimalRoute);
        ...
        private void OptimalPlacing( List<int> pVertices, List<int> pPlace)
        {
            OptimalPlacingPartEnumerate enumEngine = new OptimalPlacingPartEnumerate(fGraph, fMatrixofDistance, 
                pVertices, pPlace, positionVertexInMatrix,30000);
            enumEngine.ExecuteBody();
        ...
        //--------------------------------------------------------------------------------------
        public static int WeightOfPlacePosition(int pPosition, IMatrixOfDistance pMatrixofDistance)
        {
            int lNewWeight = 0;

            for (int j = 0; j < pMatrixofDistance.Length; j++)
            {
                lNewWeight += Math.Max(0, 5 - pMatrixofDistance[pPosition, j]);
            }
            return lNewWeight;
        }
