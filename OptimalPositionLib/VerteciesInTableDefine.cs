...
//        private bool isComplete;
        ...
        //--------------------------------------------------------------------------------------
        public string NativeRouteAsString
        {
            get
            {
...
//            baseEnumeration = pBaseEnumeration;
            ...
            if (algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedYes && algorithmConfiguration.SortedType == SortedTypeEnum.SortedBoth || algorithmConfiguration.SortedType == SortedTypeEnum.SortedVertex)
            {
                sortedVertices = sortedVertices.OrderByDescending(v => pGraph.Vertices[v].Weight).ToArray();
                sortedVerticesInverse = sortedVertices.InversePermitation();
            }

            if (algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedYes &&  algorithmConfiguration.SortedType == SortedTypeEnum.SortedBoth || algorithmConfiguration.SortedType == SortedTypeEnum.SortedPlace)
            {
            ...
            else if (algorithmConfiguration.BaseEnumeration == BaseEnumerationEnum.ByVertex)
            {
                ...
                if (algorithmConfiguration.Elimination == EliminationEnum.RughElim || algorithmConfiguration.Elimination == EliminationEnum.StrictElim)
                {
                    ...
                    if (algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedNo && algorithmConfiguration.Elimination == EliminationEnum.StrictElim)
                        ...
                        BackActionMethod = BackActionByVertexAdvSorted;
                    else if (algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedNo && algorithmConfiguration.Elimination == EliminationEnum.RughElim)
                        BackActionMethod = BackActionByVertexForced;
                    else
        ...
        public bool IsEliminableByPlaceAdv()
        {
            ...
            if (currentWeightOfRoute >= fOptimalWeight && fCurrentPosition < fNumberOfPlace)
            {
//                fCountTerminal++;
                fElemenationCount++;
                return true;
...
#if INCLUDEELIMINATION
            if ((currentWeightOfRoute >= fOptimalWeight || lPosition >= 0 &&
                Limiter(lPosition, fSize, currentCountOfEdges, countOfEdgesOnStep[lPosition],
                        currentWeightOfRoute, weightOnStep[lPosition])) && fCurrentPosition < fNumberOfPlace
                )
            {
                fCountTerminal++;
                fElemenationCount++;
...
//                fCountTerminal++;
        ...
        //--------------------------------------------------------------------------------------
        protected void BackActionByVertexForced(int pCurrentPosition)
        {
            ...
            int pCurrentWeightOfRoute, int pWeightOnStep)
        {
            ...
            if (pCurrentPosition <= 2 && algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedYes && (algorithmConfiguration.SortedType == SortedTypeEnum.SortedBoth || algorithmConfiguration.SortedType == SortedTypeEnum.SortedVertex))
            {
                if ((double)pCurrentWeightOfRoute > 0.00001)
                    return true;
            }
            if (pCurrentPosition <= 2)
            {
                if ((double)pCurrentWeightOfRoute > 2.00001)
                    return true;
            }
            if (pCurrentCountOfEdges <= 2 && algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedYes && (algorithmConfiguration.SortedType == SortedTypeEnum.SortedBoth || algorithmConfiguration.SortedType == SortedTypeEnum.SortedVertex))
            {
                if ((double) pCurrentWeightOfRoute > 0.00001)
                    return true;
            }
            if (pCurrentCountOfEdges <= 2)
            {
                if ((double)pCurrentWeightOfRoute > 2.00001)
                    return true;
            }
            if (pCurrentPosition <= 3 && algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedYes && (algorithmConfiguration.SortedType == SortedTypeEnum.SortedBoth || algorithmConfiguration.SortedType == SortedTypeEnum.SortedVertex))
            {
                if ((double)pCurrentWeightOfRoute > 1.00001)
                    return true;
            }
            if (pCurrentPosition <= 3)
            {
                if ((double)pCurrentWeightOfRoute > 2.00001)
                    return true;
            }
            if (pCurrentCountOfEdges <= 3 && algorithmConfiguration.IsSorted == IsSortedEnum.IsSortedYes && (algorithmConfiguration.SortedType == SortedTypeEnum.SortedBoth || algorithmConfiguration.SortedType == SortedTypeEnum.SortedVertex))
            {
                if ((double)pCurrentWeightOfRoute > 1.00001)
                    return true;
            }
            if (pCurrentCountOfEdges <= 3)
            {
                if ((double)pCurrentWeightOfRoute > 4.00001)
            ...
            {
                if ((double)pCurrentWeightOfRoute > 0.0001)
                    return true;
            }
            else if (pCountOfEdgesOnStep == 1)
            {
                if ((double)pCurrentWeightOfRoute > 2.0001)
                    return true;
            }
            else if (pCountOfEdgesOnStep == 2)
            {
                if ((double)pCurrentWeightOfRoute > 6.0001)
                    return true;
            }
            else if (pCountOfEdgesOnStep == 3)
            {
                if ((double)pCurrentWeightOfRoute > 10.0001)
                    return true;
            }
            else if (pCountOfEdgesOnStep == 4)
            {
                if ((double)pCurrentWeightOfRoute > 16.0001)
                    return true;
            }
